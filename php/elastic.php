...
function getQuerryString($POST) {
    ...
    // $arraydb['filter'] = var_dump($filter);
        ...
        if($POST['aggs']){
            $querry=$querry.',{  "range" : {  "time" : { "from": "'.$POST['timeFilter']['from'].'",  "to": "'.$POST['timeFilter']['to'].'"   }     }   }]}} ,"aggs": { "termsfast": { "terms": { "field": "'.$POST['aggsParam'].'.keyword","size":'.$POST['aggs'].' } } }}';  
        ...
        $querry =   '{'.$fromToString.' "query": {  "range" : {  "time" : { "from": "'.$POST['timeFilter']['from'].'",  "to": "'.$POST['timeFilter']['to'].'"   }     }   },"aggs": { "termsfast": { "terms": { "field": "'.$POST['aggsParam'].'.keyword","size":'.$POST['aggs'].' } } }}';  
    }
    else{
        if($POST['aggs']){
            $querry =   '{'.$fromToString.' "query": {  "range" : {  "time" : { "from": "'.$POST['timeFilter']['from'].'",  "to": "'.$POST['timeFilter']['to'].'"   }     }   },"aggs": { "termsfast": { "terms": { "field": "'.$POST['aggsParam'].'.keyword","size":'.$POST['aggs'].' } } }}';  
